      ...
      <div className="p-4 space-y-4 border-b bg-white sticky top-0 z-30">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
          <Button 
            ...
            size="sm" 
            className={`rounded-full h-8 px-3 text-xs gap-1 border-slate-200 ${selectedCategory === null ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'text-slate-600 hover:bg-slate-100'}`}
          ...
          {categories.map((cat: any) => (
            <Badge 
              ...
              className={`cursor-pointer shrink-0 h-8 px-3 rounded-full text-xs font-normal transition-colors ${selectedCategory === cat ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'hover:bg-slate-200 bg-slate-100 text-slate-700'}`}
                      ...
                      {umkm.umkm_photos?.[0]?.cloudinary_url ? (
                        <img 
                          ...
                          onError={(e) => {
                            ...
                            e.currentTarget.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(umkm.nama_umkm)}&background=e2e8f0&color=64748b&size=128&bold=true`;
                          }}
                        />
                      ) : (
            ...
            <div className="flex flex-col h-full">
              <div className="relative h-56 shrink-0">
                {selectedUmkm.umkm_photos?.[0]?.cloudinary_url ? (
                  <img 
                    src={selectedUmkm.umkm_photos[0].cloudinary_url} 
                    className="w-full h-full object-cover bg-slate-100" 
                    alt="" 
                    onError={(e) => {
                      e.currentTarget.onerror = null;
                      e.currentTarget.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(selectedUmkm.nama_umkm)}&background=e2e8f0&color=64748b&size=512&bold=true`;
                    }}
                  />
                ) : (
